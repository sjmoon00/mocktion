...
- **코드 리뷰 결과물도 실 데이터와 동일하게 대조 검증한다.** 코드 리뷰 결과물(자체 `/code-review`든, Copilot 등 외부 리뷰든, 이미 종합·교차검증됐다고 명시된 문서든)을 근거로 수정 계획을 세우기 전에, 그 문서가 가리키는 실제 소스 코드를 다시 열어 라인 번호와 동작 설명이 맞는지 직접 대조 확인한다. 실사용 영향이 불확실한 항목은 가능하면 실 데이터/실 DB로 재현 여부를 확인한 뒤 우선순위를 정한다 — PR #1 대응 세션에서 이미 "종합·교차검증됐다"고 적힌 리뷰 문서를 그대로 믿고 수정 계획부터 세웠다가, 실제로는 소스 대조를 하지 않았다는 지적을 받은 전례가 있다.
...
**이 계획은 채팅 응답 텍스트로 설명하는 것으로 끝내지 않는다.** 계획을 제시하는 시점에 다음 두 가지를 함께 한다 — 둘 중 하나라도 빠졌다면 아직 "계획을 세운" 것이 아니다.

1. `docs/IMPLEMENTATION_PLAN.md`에 새 Phase/Unit 섹션으로 반영한다(체크박스는 미착수 상태인 `[ ]`로). 기존 Phase 완료 후 발견된 작업이면 이어지는 번호로 새 Phase를 추가한다.
2. TaskCreate로 각 단위(+ 빌드/테스트 검증, 실사용 확인, 커밋, 문서 갱신 같은 마무리 작업까지)를 태스크로 등록한다.

계획을 사용자에게 제시하고 승인(또는 구현 진행 지시)을 받으면, 그 계획에 명시된 커밋 경계는 이미 승인된 것으로 보고 각 단위가 완성되고 검증(빌드/테스트 통과)되는 즉시 그 자리에서 커밋한다 — 전체 구현이 끝난 뒤 되돌아가서 커밋을 쪼개지 않는다. 커밋과 동시에 해당 단위의 `IMPLEMENTATION_PLAN.md` 체크박스를 `[x]`로, TaskUpdate 상태를 `completed`로 갱신한다. 계획에 없던 범위로 커밋하거나(예: 문서 갱신을 별도 계획 없이 임의로 끼워 넣는 것), 계획에 없던 시점에 커밋하는 것은 여전히 사용자 확인이 필요하다.
